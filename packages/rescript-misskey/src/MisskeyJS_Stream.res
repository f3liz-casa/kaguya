...
module Drive = {
  ...
  let use = (client: Client.t): subscription => {
    ...
    let connection = Stream_Bindings.useChannel(~stream, ~channel="drive", ())->Obj.magic
...
module ServerStats = {
  ...
  let use = (client: Client.t): subscription => {
    let stream = Client.streamClient(client)
    let connection = Stream_Bindings.useChannel(~stream, ~channel="serverStats", ())->Obj.magic
  ...
  let requestLog = (sub: subscription, ~id: string, ~length: int): subscription => {
    Stream_Bindings.ServerStats.requestLog(sub.connection, {id, length})
    sub
  }
