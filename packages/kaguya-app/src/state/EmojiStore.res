...
@val
external requestIdleCallbackWithTimeout: (
  idleDeadline => unit,
  {"timeout": int},
...
let isCacheValid = (instanceOrigin: string): bool => {
  try {
    ...
    switch metadataStr {
    | Some(str) => {
        let metadata = JSON.parseOrThrow(str)
    ...
    | Some(str) => {
        ...
        | Some(arr) => {
            ...
            arr->Array.forEach(emojiJson => {
              ...
              | Some(obj) => {
                  let name =
                    obj->Dict.get("name")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
                  let url =
                    obj->Dict.get("url")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
                  let category =
                    obj
                    ->Dict.get("category")
                    ->Option.flatMap(v => {
...
let saveToCache = (
  emojiList: array<Misskey.Emojis.customEmoji>,
  instanceOrigin: string,
): unit => {
  try {
  ...
  | error => Console.error2("EmojiStore: Failed to save to cache:", error)
  }
}
...
// Convert Misskey emoji to our emoji type
let convertEmoji = (mjEmoji: Misskey.Emojis.customEmoji): emoji => {
  {
    name: mjEmoji.name,
    url: mjEmoji.url,
...
let load = async (client: Misskey.t): result<unit, string> => {
  ...
  if currentState == Loaded {
    ...
    let cacheLoaded = if isCacheValid(instanceOrigin) {
      ...
      | Some(cachedEmojis) => {
          Console.log2(
            "EmojiStore: Loaded from cache:",
            cachedEmojis->Dict.keysToArray->Array.length,
          )
    ...
    if cacheLoaded {
      ...
      | Ok(emojiList) => {
          ...
          Console.log2(
            "EmojiStore: Total emoji entries (with aliases):",
      ...
      | Error(msg) => {
          Console.error2("EmojiStore: Global emoji load failed:", msg)
...
let rec preloadEmojiImages = (batchSize: int, startIndex: int): unit => {
  ...
  if startIndex >= totalEmojis {
    ...
    Console.log3("EmojiStore: Preloaded images", endIndex, "/" ++ totalEmojis->Int.toString)

    // Schedule next batch during idle time with much longer delay
    // Use a 3 second timeout to avoid spamming requests
    if supportsIdleCallback() {
      let _ = requestIdleCallbackWithTimeout(_deadline => {
