...
// history.back() binding — used by the PWA back button
@val @scope("history")
external historyBack: unit => unit = "back"

// history.length binding — check if we can actually go back
@val @scope("history")
external historyLength: int = "length"
...
let make = (~children: Preact.element) => {
  ...
  // Determine if we're on a root-level navigation page.
  // Sub-pages (notes, user profiles, etc.) show a back button since PWA
  // standalone mode has no browser navigation controls.
  let isRootPage =
    location == "/" ||
  ...
  let handleBack = (_: JsxEvent.Mouse.t) => {
    if historyLength > 1 {
      historyBack()
    } else {
      navigate("/")
