  ...
  let filteredEmojis = allEmojis->Array.filter(emoji => {
    ...
    if isLikeOnly {
      ...
      let matchesSearch = if searchQuery == "" {
        ...
        let aliasMatches = emoji.aliases->Array.some(alias => 
          alias->String.toLowerCase->String.includes(query)
        )
    ...
    >
      ...
      {if !isLikeOnly && categories->Array.length > 1 {
        <div className="emoji-categories" role="tablist">
          <button
            className={selectedCategory->Option.isNone ? "emoji-category-tab active" : "emoji-category-tab"}
      ...
      <div className="emoji-grid" role="grid" ariaLabel="Emoji list"
        onScroll={e => {
          let target = JsxEventU.UI.target(e)
      ...
      >
        {if filteredEmojis->Array.length == 0 {
          ...
          >
            <div
              className="emoji-grid-items"
              style={itemsStyle}
            >
              {visibleEmojis->Array.map(emoji => {
                <button
                  key={emoji.name}
