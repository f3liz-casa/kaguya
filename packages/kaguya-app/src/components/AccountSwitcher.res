...
// AccountSwitcher.res - Dropdown to switch between accounts
...
let make = () => {
  ...
  let handleSwitch = (accountId: string) => {
    ((_: JsxEvent.Mouse.t) => {
      setIsOpen(_ => false)
      let _ = AppState.switchAccount(accountId)
    })
  }
  ...
  let handleAddAccount = (_: JsxEvent.Mouse.t) => {
    setIsOpen(_ => false)
    // Set a flag so login page knows to add, not replace
    navigateWithOptions("/add-account", {replace: false})
  }

  // Get active account info for trigger display
