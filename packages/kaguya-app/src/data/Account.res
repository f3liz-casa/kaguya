...
// Account.res - Multi-account data model
...
let permissionModeToString = (mode: permissionMode): string => {
  switch mode {
  | ReadOnly => "ReadOnly"
  | Standard => "Standard"
  }
...
let permissionModeFromString = (s: string): permissionMode => {
  switch s {
  | "ReadOnly" => ReadOnly
  | _ => Standard
  }
