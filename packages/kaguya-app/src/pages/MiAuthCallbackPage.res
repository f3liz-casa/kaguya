...
let make = () => {
  ...
  // Track if we've already started the check (prevent duplicate calls on re-render)
  let hasStartedCheck = ref(false)

  // Check MiAuth session on mount and set up polling
  // Use an empty dependency array to ensure this only runs once
  PreactHooks.useEffect0(() => {
    // Prevent duplicate checks if component re-renders
    if hasStartedCheck.contents {
      Console.log("MiAuthCallbackPage: Check already started, skipping...")
      None
    } else {
      ...
      let checkAuth = async () => {
        try {
          if !isMounted.contents {
            ...
            Console.log2("MiAuthCallbackPage: Result is Ok?", switch result {
            | Ok() => true
            | Error(_) => false
            })
      ...
      <article className="login-card">
        ...
        | "success" =>
          <div className="success-message">
            ...
            <p style={Style.make(~fontSize="0.875rem", ~fontWeight="400", ~marginTop="0.5rem", ~color="#158033", ())}> 
