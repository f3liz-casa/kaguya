...
// Parse, validate, and bundle an OpenAPI spec from URL or file path
// NOTE: We use bundle instead of dereference to preserve $ref strings.
// Dereferencing creates circular JavaScript objects for recursive schemas
// (like Note.reply -> Note), which causes exponential parsing time.
let resolve = async (source: string, ~timeout: option<int>=?): result<Types.openAPISpec, string> => {
  try {
    ...
    | Some(ms) => {timeoutMs: ms}
    | None => {timeoutMs: 60000} // 60 second default
    }
  ...
  | JsExn(err) => {
      ...
      Result.Error(`Failed to resolve spec: ${message}`)
    }
  }
}
...
let parseOnly = async (source: string, ~timeout: option<int>=?): result<Types.openAPISpec, string> => {
  ...
  | JsExn(err) => {
      ...
      Result.Error(`Failed to parse spec: ${message}`)
    }
  }
}
...
let bundleSpec = async (source: string, ~timeout: option<int>=?): result<JSON.t, string> => {
  ...
  | JsExn(err) => {
      ...
      Result.Error(`Failed to bundle spec: ${message}`)
    }
  }
}
...
let validateSpec = async (source: string, ~timeout: option<int>=?): result<Types.openAPISpec, string> => {
  ...
  | JsExn(err) => {
      ...
      Result.Error(`Failed to validate spec: ${message}`)
    }
  }
}
