...
let extractSharedEndpoints = (
  ...
  // Find base endpoints that exist in fork and are unchanged
  baseEndpoints->Belt.Array.keep(baseEp => {
    let key = makeEndpointKey(baseEp)
    switch Dict.get(forkMap, key) {
    | Some(forkEp) => {
        // Check if they're the same (use simple comparison for now)
        switch SpecDiffer.compareEndpoints(baseEp, forkEp) {
        | None => true // No differences
        | Some(_) => false // Has differences
        }
      }
    | None => false
    }
...
// Extract fork-specific endpoints (added or modified in fork)
let extractForkExtensions = (
  ...
  // Find endpoints in fork that are new or different
  forkEndpoints->Belt.Array.keep(forkEp => {
    let key = makeEndpointKey(forkEp)
    switch Dict.get(baseMap, key) {
    | None => true // New endpoint
    | Some(baseEp) => {
        // Check if modified
        switch SpecDiffer.compareEndpoints(baseEp, forkEp) {
        | None => false // Same as base
...
// Extract shared component schemas
let extractSharedSchemas = (
  ...
  | (None, _) | (_, None) => None
  | (Some(base), Some(fork)) => {
      let shared = Dict.make()
      
      Dict.keysToArray(base)->Belt.Array.forEach(name => {
        switch (Dict.get(base, name), Dict.get(fork, name)) {
        | (Some(baseSchema), Some(forkSchema)) => {
            // If schemas are equal, it's shared
            if SpecDiffer.schemasEqual(baseSchema, forkSchema) {
              Dict.set(shared, name, baseSchema)
            }
          }
        | _ => ()
        }
      ...
      if Dict.keysToArray(shared)->Belt.Array.length > 0 {
        Some(shared)
      } else {
        None
      }
...
let extractForkSchemas = (
  ...
  | (Some(base), Some(fork)) => {
      ...
      Dict.keysToArray(fork)->Belt.Array.forEach(name => {
        switch (Dict.get(base, name), Dict.get(fork, name)) {
      ...
      if Dict.keysToArray(extensions)->Belt.Array.length > 0 {
        Some(extensions)
      } else {
        None
      }
...
let mergeWithSharedBase = (
  ...
  let baseSchemas = baseSpec.components->Belt.Option.flatMap(c => c.schemas)
  let forkSchemas = forkSpec.components->Belt.Option.flatMap(c => c.schemas)
...
let getMergeStats = (
  ...
  {
    sharedEndpointCount: Belt.Array.length(shared),
    forkExtensionCount: Belt.Array.length(extensions),
    sharedSchemaCount: sharedSchemas->Belt.Option.mapWithDefault(0, s => 
      Dict.keysToArray(s)->Belt.Array.length
    ),
    forkSchemaCount: extensionSchemas->Belt.Option.mapWithDefault(0, s => 
      Dict.keysToArray(s)->Belt.Array.length
    ),
  }
}
